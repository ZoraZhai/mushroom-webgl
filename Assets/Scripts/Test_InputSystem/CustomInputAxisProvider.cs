...
namespace Cinemachine
{
    ...
    public class CustomInputAxisProvider  : MonoBehaviour, AxisState.IInputAxisProvider
    {
        /// <summary>
        ...
        {
            if (InputPinchZoom.isZooming == true)
            {
                return 0;
            }
            ...
            if (enabled )
            {
                ...
                //Debug.Log(action.ReadValue<Vector2>().x);
                if (action != null)
                {
                    switch (axis)
                    {
                        case 0: return action.ReadValue<Vector2>().x* rotateSpeed;
                        case 1: return action.ReadValue<Vector2>().y* rotateSpeed;
        ...
        public InputAction ResolveForPlayer(int axis, InputActionReference actionRef)
        {
